...
public class LoggerUtility {
    ...
    public static void logError(String message) {
        ...
        showAlert(AlertType.ERROR, "Fejl", message); // Viser fejlbeskeden i UI
    }
    ...
    public static void logWarning(String message) {
        ...
        showAlert(AlertType.WARNING, "Advarsel", message); // Viser advarslen i UI
    }
    ...
    // Viser en alert dialog i SceneBuilder UI
    private static void showAlert(AlertType alertType, String title, String message) {
        Platform.runLater(() -> { // Sørger for at koden kører i JavaFX-tråden
            Alert alert = new Alert(alertType); // Opretter en alert af given type
            alert.setTitle(title); // Sætter alert-titel
            alert.setHeaderText(null); // Ingen header-tekst
            alert.setContentText(message); // Sætter beskeden
            alert.showAndWait(); // Viser alert og venter på brugerens handling
        });
    }
}
